...
-->
      ...
      <!-- Side Nav END -->
...

        {
          ...
          $sel2="SELECT * FROM comments WHERE Episode_id='$episode_id'";
          $run_comment=mysqli_query($conn,$sel2);
          $num_comment=mysqli_num_rows($run_comment);

          $fetch_episode=mysqli_fetch_array($run_episode);
          
        

          $episode= $fetch_episode['Episode'];
          $title= $fetch_episode['Title'];
          $date=$fetch_episode['_Date'];
          $picture=$fetch_episode['Picture'];
          $content=$fetch_episode['Content'];
        
   $display=" ";

   //this is for confessiong
   $week= date("W");

          $sel="SELECT * FROM question WHERE Week='$week'";
          $run_question=mysqli_query($conn,$sel);
          $fetch_question=mysqli_fetch_array($run_question);
          

          $sel2="SELECT * FROM confess WHERE Week='$week'";
          $run_confess=mysqli_query($conn,$sel2);
          $num_confess=mysqli_num_rows($run_confess);
          ...
          $date_confess= $fetch_question['_Date'];
...
*/
...

        {
          ...
          $sel="SELECT * FROM question WHERE Question_id='$question_id'";
          $run_question=mysqli_query($conn,$sel);
          $fetch_question=mysqli_fetch_array($run_question);
          $week=  $fetch_question ['Week'];

          $sel2="SELECT * FROM confess WHERE Week='$week'";
          $run_confess=mysqli_query($conn,$sel2);
          $num_comment=mysqli_num_rows($run_confess);
          ...
          <div class="main-content">
            ...
            <div class="container-fluid">
              <div class="row">
                ...
                <div class="col-12 col-xl-6 m-b-10" style="display:<?php echo $display ;?>">
                 <div class="card">   <div class="card-header border-bottom">
                      <h4 class="card-title">Episode</h4>
                    </div>
<?php echo"<img class='img-fluid' style='height:auto;width:100%'
         src='uploads/$picture' alt=''> ";?>
                    ...
                    <div class="card-body">
                      <p class="text-muted m-b-20 box-content">
                        <?php echo $content; ?> <br>
                        <small class='day'><?php echo $date ?></small>
                      </p>
                      <hr>
                      
                      <div class="table-responsive">
                      <span class='glyphicon glyphicon-comment'></span> <?php if($num_comment==1)
                      {
                        echo $num_comment ." "."Comment";
                      }
                      else{
        ...
        <?php
          ...
          while( $fetch_comment=mysqli_fetch_array($run_comment))

          {

            $sex=$fetch_comment['Sex'];
            $live=$fetch_comment['Live'];
            $name=$fetch_comment['Name'];
            $comment=$fetch_comment['Comment'];
            $email=$fetch_comment['Email'];
            $date=$fetch_comment['_Date'];
            $comment_id=$fetch_comment['Comment_id'];
            if($live==0){


              $share="<button class='btn btn-default' type='submit' 
              name='share_comment' value='$comment_id' >
              <span class='glyphicon glyphicon-share'></span> Share Comment
              </button>";

            }
            elseif($live==1){

              $share="<button class='btn btn-default' type='submit' name='remove_comment' value='$comment_id'>
              <span class='glyphicon glyphicon-remove-circle'></span> Remove Comment
              </button>";
            ...
            if($sex=="M")
            {
              ...
              <!--FIFTH LIST ITEM-->
              <li class='media'> <span class='round'>
          ...
          ?>
                ...
                <div class="col-12 col-xl-6 m-b-10">
                  <div class="card">
                    <div class="card-header border-bottom">
                      <h4 class="card-title">Confession</h4>
                    </div>
                    <div class="card-body">
                      <p class="text-muted m-b-20 box-content">
                      ...
                      echo $question; }
                        ...
                        echo"<h4 class='text-danger'>You have no content this week</h4>";
                    ...
                    <div class="card-body">
                      <p class="text-muted m-b-20 box-content">
                        ...
                        <code><?php echo $date_confess; ?></code>.
                      </p>
                      <hr>
               <?php       while( $fetch_confess=mysqli_fetch_array($run_confess))

{
  $question_id=$fetch_confess['Question_id'];
  $sex=$fetch_confess['Sex'];
  $live=$fetch_confess['Live'];
  $name=$fetch_confess['_Name'];
  $confess= $fetch_confess['Confession'];
  $confess_id=$fetch_confess['Confess_id'];
  $date=$fetch_confess['_Date'];
  if($live==0){


    $share="<button class='btn btn-default' type='submit' name='share_confession' value='$confess_id' >
    <span class='glyphicon glyphicon-share'></span> Share Confession
    </button>";

  }
  elseif($live==1){

    $share="<button class='btn btn-default' type='submit' name='remove_confession' value='$confess_id'>
    <span class='glyphicon glyphicon-remove-circle'></span> Remove Confession
    </button>";
    ...
    <img src='https://img.icons8.com/office/100/000000/user-group-woman-woman.png'
     ...
     <div class='media-body'>
            <div class='row d-flex'>
                ...
                <p class='text text-centre'> $sex</p> 
                <div class='ml-auto'>
                    <code> $date</code>
                </div>
            </div>
